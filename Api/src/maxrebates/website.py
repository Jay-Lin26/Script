...
class PriceComparisonWeb:
    ...
    def thecashback(self):
        ...
        try:
            ...
            if store_data == ' ' or sign_up_url == ' ':
                self.msg.execution('TheCashBack.kr数据异常, 请及时处理; 数据返回为空')
            elif store_data != " ":
                for i in range(len(store_data)):
                    rebate = store_data[i]["rebate"]
                    if re.match("^([^\d]*)0([^\d]*) Cash Back", rebate) is not None:
                        self.msg.execution("[Thecashback] 商家 {}".format(store_data[i]["name"]) + "({})".format(store_data[i]["rebate"]) + " 数据异常！")
            # self.msg.execution("执行完毕！！Thecashback未发现异常数据")
        except IndexError:
            self.msg.execution('TheCashBack.kr数据异常, 请及时处理')
        except KeyError:
            self.msg.execution('TheCashBack.kr数据异常, 请及时处理')
    ...
    def holic(self):
        ...
        try:
            store_data = r["list"]
            if store_data == ' ':
                self.msg.execution('TheCashBackHolic数据异常, 请及时处理')
            elif store_data != " ":
                for i in range(len(store_data)):
                    rebate = store_data[i]["rebate"]
                    if re.match("^([^\d]*)0([^\d]*) Cash Back", rebate) is not None:
                        self.msg.execution("[holic] 商家 {}".format(store_data[i]["name"]) + "({})".format(
    ...
    def monitor(self):
        ...
        try:
            ...
            for i in range(len(storeRebate)):
                ...
                if re.match("^([^\d]*)0([^\d]*) Cash Back", rebate) is not None:
                    self.msg.execution("[Thecashback] 商家 {}".format(storeRebate[i]["name"]) + "({})".format(
                        storeRebate[i]["rebate"]) + " 数据异常！")
            # self.msg.execution("执行完毕！！Monitor未发现异常数据")
        except IndexError:
            self.msg.execution('monitor数据异常, 请及时处理')
        except KeyError:
            self.msg.execution('monitor数据异常, 请及时处理')
